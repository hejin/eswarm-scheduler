...
// This file was copied from the noderesources scheduler of the main k/k repo and defaultResourcesToWeightMap was added.
...
type resourceAllocationScorer struct {
	Name                string
	scorer              func(useCpuPerfRatio bool, requested, allocatable resourceToValueMap) int64
...
// return the label 'emulate-arm' on Node
func (r *resourceAllocationScorer) getNodeCpuPerfLabel(
...
func (r *resourceAllocationScorer) score(
	...
	// Get the node architecture label
	useCpuPerfRatioFlag, _ := r.getNodeCpuPerfLabel(nodeInfo)
	// FIXME: handle the 2nd return value
	useCpuPerfRatio := false
	if useCpuPerfRatioFlag == "Y" {
		useCpuPerfRatio = true
	}
	klog.InfoS("eswarm scheduler: found ", "node", node.Name, "'s emulate-arm label = ", useCpuPerfRatioFlag)

	score := r.scorer(useCpuPerfRatio, requested, allocatable)
